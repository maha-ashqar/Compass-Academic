...
import {
  ...
  FiFilter, FiPlus, FiSearch, FiStar, FiTarget, FiTrash2, FiUsers, FiX,
} from 'react-icons/fi';
...
function TrainerCompetitions() {
  ...
  const filtered = useMemo(() => competitions.filter((item) => (filter === 'all' || item.phase === filter) && `${item.title} ${item.category}`.toLowerCase().includes(query.toLowerCase())), [competitions, filter, query]);
